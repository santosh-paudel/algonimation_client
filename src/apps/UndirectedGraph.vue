      ...
      <div class="w-100 h-100">
        ...
        <aa-modal-box
          ...
          @on-submit="
            doGraphOperation(
              {
                source: graphOperations,
                target: graphOperationSelectedNode2
              },
              this.selectedGraphOperation
            )
          "
          @on-closed="showModalBox = false"
        >
...
export default {
  ...
  computed: {
    ...
    clickedRelativeCoord() {
      return {
        x: this.clickCoord.x,
        y: this.clickCoord.y + this.$refs["graph-operation-panel"].clientHeight
      };
    },
  ...
  methods: {
      ...
      switch (actionName) {
        case "Dikstra's shortest path": {
          if (userInput === null) {
            ...
            this.graph.dikjstras(userInput.key1, userInput.key2);
          }
          break;
        }
    ...
    generateRandomGraph() {
      ...
      this.graph.link("b", "c", 2);
      this.graph.link("b", "e", 6);
      this.graph.link("c", "d", 1);

      this.graph.link("b", "d", 1);
      this.graph.link("d", "e", 1);
      /*
