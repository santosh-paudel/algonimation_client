...
class Graph {
  ...
  /**
   * Given the starting and ending Node, it returns the shortest path
   * between those two nodes (if any)
   */
  dikjstras(key1, key2) {
    ...
    for (let key of Object.keys(this._database)) {
      if (key === key1) {
        pathTable[key] = { cost: 0, previous: null };
      } else {
        pathTable[key] = { cost: Math.max(), previous: null };
      }
    }
    ...
    while (currentNodeKey !== null) {
      ...
      for (let neighbor of Object.keys(this._adjacencyList[currentNodeKey])) {
        ...
        if (!visited[neighbor] && newCost < pathTable[neighbor].cost) {
        ...
        //  |  /                |
        // Node(d)------------Node(e)
        if (!visited[neighbor] && newCost < minCost) {
          minCost = newCost;
