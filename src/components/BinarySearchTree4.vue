...
export default {
	...
	methods: {
		...
		computeXCoordinate(node) {
			if (node.parent === null) {
				throw Error(
					"The node needs to have a parent to compute x coordinate"
				);
			}
			...
			if (node.parent.parent === null) {
				if (node.data.data <= node.parent.data.data) {
					return node.parent.rightChild() === null
						? node.parent.x - 100
						: node.parent.x -
								(node.parent.rightChild().x - node.parent.x);
				} else {
					return node.parent.leftChild() === null
						? node.parent.x + 100
						: node.parent.x +
								(node.parent.x - node.parent.leftChild().x);
				}
			} else if (node.data.data <= node.parent.data.data) {
				return (
					node.parent.x -
					Math.abs(node.parent.x - node.parent.parent.x)
				);
			} else {
				return (
					node.parent.x +
