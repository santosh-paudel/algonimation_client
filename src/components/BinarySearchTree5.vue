  ...
  methods: {
    ...
    insert: async function() {
      ...
      if (this.bstD3Wrapper.height() > 1) {
        let ancestors = this.bstD3Wrapper.getPathToParentById(node.data.id);
    ...
    remove: async function() {
      ...
      await this.traverse(nodes, true);
      debugger;

      // If nodes is not empty, remove the node from DOM
      let removalNode = null;
      if (nodes.length > 0) {
        removalNode = nodes[nodes.length - 1];

        //Remove the node
        this.removeElement(removalNode.data.id, this.animationTime600);

        //Remove the link
        this.removeElement(this.getLinkId(removalNode), this.animationTime600);
      }

      //Now, remove the node from the actual tree itself
      if (removalNode != null) {
        this.bstD3Wrapper.removeNodeById(removalNode.data.id);
      }
    },
